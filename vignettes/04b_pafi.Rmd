...
```{r}
set.seed(123); res <- pafi(dolphins, flips = 100, rand = 100, progbar = F)
...
```{r}
xdata <- dolphins[, -1]
set.seed(123); pafi(xdata, flips = 100, rand = 100, progbar = F,
     exclcols = c("B", "C", "D", "I", "J", "K", "L", "M", "N", "O", "P", "Q", "R"))
```
...
```{r}
set.seed(123); withpres <- pafi(asso, presence = pres, progbar = F)
